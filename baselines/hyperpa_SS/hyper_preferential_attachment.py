...
class hyper_preferential_attachment:
    ...
    def learn_size_distribution(self):
        ...
        g = open(self.size_distribution_directory + "/" + self.name + ".txt")
        gl = g.readlines()
        size_distribution = []
        for line in gl:
    ...
    def learn_number_simplices_per_node(self):
        g = open(self.simplex_per_node_directory + "/" + self.name + ".txt", "r")
        count = -1
        ...
        g = open("simplex per node/" + self.name + ".txt", "r")
    ...
    def generate(self):
        ...
        self.size = min(24, self.num_nodes)
        self.current_num_nodes = self.size
        ...
        for i in range(12):
            f.write(str(i + 1) + " " + str(self.size-i) + "\n")
            self.deg_list[0][str(i+1) + "," + str(self.size-i)] = 1
        ...
        for node in range(self.size, self.num_nodes+1):
            hyper_edges = []
