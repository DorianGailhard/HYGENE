...
class NodeNumDiff(Metric):
    ...
    def __call__(self, reference_graphs, predicted_graphs, train_graphs):
        ref_node_num = np.array([G.number_of_nodes()
                                for G in reference_graphs])
        pred_node_num = np.array([G.number_of_nodes()
...
class Wavelet(Metric):
    ...
    def __call__(self, reference_graphs, predicted_graphs, train_graphs):
        reference_eigvals, reference_eigvecs = compute_list_eigh(
            reference_graphs)
        predicted_eigvals, predicted_eigvecs = compute_list_eigh(
            predicted_graphs)
    ...
    def __call__(self, reference_graphs, predicted_graphs, train_graphs):
        ...
        if reference_graphs_key in self.cache:
            ...
            train_metrics = np.array(
                [m(reference_graphs, train_graphs, train_graphs)
                 for m in metrics]
            )
            self.cache[reference_graphs_key] = train_metrics

        predicted_metrics = np.array(
            [m(reference_graphs, predicted_graphs, train_graphs)
             for m in metrics]
        )
...
class ValidBipartite(Metric):
    ...
    def __call__(self, reference_graphs, predicted_graphs, train_graphs):
        ...
        for G in predicted_graphs:
            bipartiteNum += nx.algorithms.bipartite.is_bipartite(G)
        return bipartiteNum / len(predicted_graphs)


class UniqueNovelValidBipartite(Metric):
    def __str__(self):
        return "UniqueNovelValidBipartite"

    def __call__(self, reference_graphs, predicted_graphs, train_graphs):
        return eval_fraction_unique_non_isomorphic_valid(
            predicted_graphs, train_graphs, nx.algorithms.bipartite.is_bipartite
        )[2]